      ...
      case 'UPDATE_DETAILS':
        ...
        case 'UPDATE_LOCATION':
          ...
          case 'RESET_ROOM':
            ...
            case 'UPDATE_ROOMS':
              return{
                ...state,rooms:action.payload, addressFilter:null, priceFilter:50000, filteredRooms:action.payload
              };
            ...
            case 'FILTER_ADDRESS':
              return {...state,addressFilter:action.payload, filteredRooms: applyFilter(
                state.rooms, state.addressFilter, action.payload,state.priceFilter
              )}

            case 'CLEAR_ADDRESS':
              return {...state,addressFilter:null ,priceFilter: 50000, filterteredRooms:state.rooms };
...
const applyFilter = (rooms, address, price)=>{
  ...
  if(address){
    ...
    filteredRooms = filteredRooms.filter(rooms=>{
      const lngDifference= lng>rooms.lng?lng-rooms.lng:rooms.lng-lng

      const latDifference= lat>rooms.lat?lat-rooms.lat:rooms.lat-lat
